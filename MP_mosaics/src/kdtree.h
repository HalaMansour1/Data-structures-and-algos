...
class KDTree
{
  ...
  private:
    ...
    KDTreeNode* buildTree(std::vector<Point<Dim>>& points, int depth, int start, int end);
    KDTreeNode* copyhelper(const KDTreeNode* node);
    void deleter(KDTreeNode*& current);
    Point<Dim> Nearesthelper(KDTreeNode* node, const Point<Dim>& query, int depth) const;
    bool Replace(const Point<Dim>& target, const Point<Dim>& currentBest, const Point<Dim>& potential) const;
